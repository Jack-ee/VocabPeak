...
//     (bumpDaily, markActiveDay) / 薄弱词 (recordWordResult) 一律不
//     落账, 家长后台数据保持干净。pref device_test_mode 在同步黑名
//     单里 —— 仅本机生效, 绝不同步 (同步出去孩子平板就全家不记了)。
//     历史已混入的数据无法回溯区分, 开关只管往后。
//   • 家长后台课表碎屑过滤: 应用启动自动恢复旧课会记 1 分钟级碎屑
//     (孤零零的"1 分"幽灵行)。单课单日不足 60 秒的贡献不进课表累计、
//     不刷新"最近学习"; 天表保持全量, 总分钟不变。
//   • 课表新增"今日 读/练"列: 一眼区分"今天干的"与"累计攒的",
//     消除"只学一课却四门课都有当天日期"的误读。
...
//   • 新行为 —— 闯关语义: 答对从本轮词单划掉 (徽标立减, 进度看得
//     见); 答错挪到队尾稍后重考 (原本在最后一位时归零起步, 避免
//     连考同一词); 全部消灭 → 🎉 完成提示 (答对 X/Y) 并回到全部
//     筛选, 想再来一组点药丸重抽。
//   • 「毕业」与「划掉」是两层: 毕业是长期身份 (连对>=4 且跨>=3
//     学习日, 不随时间衰减), 划掉只是本轮进度 —— 本轮答对照常累积
//     长期毕业进度。
//   • 徽标语义: 特训进行中 = 本轮剩余; 非特训 = 长期待训池 (三路
//     聚合口径不变)。

// hsv-v38 (?v=135) — 特训入口补全 (单词页):
//   • 单词页筛选药丸行新增 🎯 特训 (紧跟 🔁 复习), 带待训数徽标 ——
//     练的就是单词, 孩子在单词页不该绕回课文页找入口。
//   • 点击特判走 startTrainer (抽词单 + 直进测验), 不是普通筛选;
//     词单为空只提示不切换。active 高亮由 render 的 studyFilter 同步
//     逻辑自动处理。
//   • 待训数与课文页入口卡同一聚合口径, 答题中实时变化 —— 毕业一个
//     少一个, 数字往下走本身就是反馈。

// hsv-v37 (?v=134) — 薄弱词闭环: 统一记账 + 严格毕业 + 今日特训:
//   • 统一记账 (DB.recordWordResult): 单词测验与课文填空的对错全走
//     一处 —— weak 打标 / 计数 / 毕业口径全局一致。此前课文答错只拉
//     SRS 不打 weak 标签, 课文答对不推动毕业, 两线各记各的。课文
//     答错的 SRS 拉回职责也并入 (reinforceWord 不再单独调
//     flagQuizMistake, 避免双计)。
//   • 严格毕业 (家长定的规则, 别放宽): 薄弱词只因「真实掌握」退出,
//     绝不因时间流逝退出 —— 学期中几个月没空练, 回来它必须还在。
//     掌握 = 连对 >= 4 次且分布在 >= 3 个不同学习日 (streakDays 字段);
//     一天内刷出的连对不算, 隔天还记得才算。答错清零重攒; 攒到一半
//     长期不练, 进度原样保留。
//   • 今日特训 (零决策入口): 课文列表页激励卡上的 🎯 按钮, 三路聚合
//     选词 (weak 标签 +4 / 课文最近答错 +3 / 复习过且到期 +2, 错次
//     微调排序) 取前 12, 一键切到单词页直接进测验; 答题自动走统一
//     记账与毕业。词单会话内固定, 不因答题中途缩水。
//   • weak 标签一次性迁移: 合流前课文错过的词 (有 mistakeCount 无
//     标签) 补打 weak, 进入统一毕业流程; 幂等。
//   • 家长后台口径更新: 薄弱词只认 weak 标签 (毕业即退出本区);
//     chip 显示"N 天未练" (>=7 天), 长期冷落一眼可见。
