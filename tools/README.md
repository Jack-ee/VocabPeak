...
以教材课文为单位的四步学习闭环：听读（逐句高亮播放，整句音频预置在离线发音包中，无英文系统语音的平板也能整篇朗读）→ 点词（蓝色标注词弹卡：原型、词形、释义、原句、常用搭配，一键入生词本进 SRS）→ 填空（原文句子挖空，4 选 1 或键入拼写，错词可批量入生词本，答题计入每日测验量）→ 短语（高频搭配浏览与中英匹配练习）。语料在 lessons-data.js，按课扩展。
...
| lessons.js / lessons-data.js / lessons.css | 课文精读：模块、语料、样式 |
